    ...
    def process(self, image: NumpyImage, general: GeneralData, iter_index: int, seed: int) -> NumpyImage:
        if (result := shared.backend.image_to_image(
            ...
            copy_with_overrides(self.parameters,
                positive_prompt = evaluate_prompt(self.parameters.positive_prompt, iter_index - 1, seed),
                negative_prompt = evaluate_prompt(self.parameters.negative_prompt, iter_index - 1, seed),
                seed = seed,
            ),
